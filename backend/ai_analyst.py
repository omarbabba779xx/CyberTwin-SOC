...
class AIAnalyst:
    ...
    # ------------------------------------------------------------------
    # Evidence-first analysis (Phase 3 - structured output)
    # ------------------------------------------------------------------
    ...
    def analyse_with_evidence(
        ...
        # Confidence: deterministic. Anchored to alert volume + tactic diversity.
        if not alerts:
            confidence = 0.0
        else:
        ...
        # Hypotheses are *only* offered when confidence is too low to claim a
        # firm conclusion. They are explicitly framed as hypotheses, never as
        # facts.
        hypotheses: list[str] = []
        ...
        # Recommended actions: derive from existing immediate_actions but make
        # sure each one references an alert or IOC, never a fabricated detail.
        recommended_actions = self._evidence_grounded_actions(alerts, incidents)

        # Limitations: be explicit when the analysis is constrained.
        ...
        # Summary: tie the headline claim to the evidence count, never overclaim.
        if alerts:
            summary = (
        ...
        # IOCs we already know about, never fabricated.
    ...
    def _collect_evidence(
        ...
        for alert in alerts:
            ...
            for ev in alert.get("matched_events", []) or []:
                # Pick the most informative field present
                for field, label in (
